...
const tyrannosaurus = {  
  ...
  const unisWithUni = [];
    ...
    graduates.forEach(checkU);
  
   function checkU(value) {
    if(value.university.includes('Uni')){unisWithUni.push(value)}
  }
  ...
  */
  ...
  var pops = [];
  zooAnimals.forEach(getPops);
  
  function getPops(item, index) {
    pops.push(item.population);
  }
  const reducer = (accumulator, currentValue) => accumulator + currentValue;
  populationTotal = pops.reduce(reducer,0);
